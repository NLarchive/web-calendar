...
import { SORT_MODES, DEFAULT_CALENDAR_COLOR } from '../../core/constants.js';
...
export function renderAppointmentList(root, appointments, calendarColorMap = new Map(), onAppointmentClick) {
  ...
  if (!sorted.length) {
    root.innerHTML = `<p class="small">${t('list.noAppointmentsYet')}</p>`;
    return;
  }
  ...
  root.innerHTML = `<div class="list">${sorted
    .map(
      (item, index) => `
          <article class="calendar-item" style="border-left: 4px solid ${calendarColorMap.get(item.calendarId || 'default') || DEFAULT_CALENDAR_COLOR};">
          <button
            class="calendar-item-trigger"
            data-action="open-appointment-details"
            data-appointment-key="${item.id || index}"
            aria-label="${t('list.openAppointmentDetails')}"
          >
            <h4>${escapeHtml(item.title)}</h4>
            <div class="small">${formatDateTime(new Date(item.date), { timeZone: item.timezone, includeTimeZone: true })}</div>
            <div class="small">${escapeHtml(item.category || t('list.general'))} • ${t('list.priority')} ${item.priority} • ${escapeHtml(item.calendarId || 'default')}</div>
          </button>
        </article>
      `,
    )
